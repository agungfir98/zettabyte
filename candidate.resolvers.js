async function UpdateCandidate(
  ...
  // REFACTOR START: serializing input
  serializeInput(candidate_input) // refactor
  // do I have to delete the artifact ????
  ...
  // REFACTOR END: serializing input


  // REFACTOR START: validating parent Iban
  if (candidate_input.parents && candidate_input.parents.length) {
    await validateParentIban(candidate_input.parent) // refactor
  ...
  // REFACTOR END: validating candidate Iban


  // REFACTOR START: validating payment support
  if (candidate_input.payment_supports && candidate_input.payment_supports.length) {
    await validatePaymentSupports(payment_supports) // refactor
  ...
  // REFACTOR END: validating payment support
  ...
  // REFACTOR START: isCandidateRegistered
  // To update candidate status, and validation on readmission assignment table
  if (
    isCandidateRegistered(candidate_input, oldCandidate) // refactored
    // REFACTOR START: isCandidateRegistered
  ) {
  ...
  // REFACTOR START: isCandidateRegistered
  if (
    isCandidateRegistered(candidate_input, oldCandidate)
  ) {
    const candidateFound = await CandidateModel.findById(oldCandidate._id).select('student_id').lean();
    const otherCandidateSameStudent = await CandidateModel.find({ student_id: candidateFound.student_id }).select('_id').lean();
...
async function validateParentIban(parents) {
...
async function validateCandidateIban(candidate_input) {
  const ibanHistory = await IbanHistoryModel.create({
    candidate_id: _id,
    iban: candidate_input.iban,
    bic: candidate_input.bic,
...
async function validatePaymentSupports(payment_supports) {
...
function setParentalLink(parentalLink, relations) {
  return parentalLink === 'other' ? '' : relations.includes(parentalLink) ? 'MR' : 'MRS';
}


function isCandidateRegistered(candidate_input, oldCandidate) {
  return (candidate_input.candidate_admission_status &&
    oldCandidate.candidate_admission_status === 'registered' &&
