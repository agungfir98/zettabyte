...
type Session = typeof sessions
function formatSession(sessions: Session) {
  const sessionMap = new Map()


  for (const { class: cls, session_id, student, time } of sessions) {
    ...
    if (!session.classes.has(cls.class_id)) {
      session.classes.set(cls.class_id, { class_id: cls.class_id, name: cls.name, students: [], })
    }

    const classObj = session.classes.get(cls.class_id)
  ...
  // return sessionMap
  return Array.from(sessionMap.values()).map(session => ({ ...session, classes: Array.from(session.classes.values()) }))
}

// console.log(formatSession(sessions))
console.log(JSON.stringify(formatSession(sessions)))
